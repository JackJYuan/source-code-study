...
public class ContextLoader {
	...
	static {
		...
		try {
			/**
			 * 1. 根据DEFAULT_STRATEGIES_PATH和ContextLoader.class构建ClassPathClassPathResource
			 * path为相对路径，全路径为：org.springframework.web.context.ContextLoader.properties
			 * */
			ClassPathResource resource = new ClassPathResource(DEFAULT_STRATEGIES_PATH, ContextLoader.class);
			// 2.加载resource的属性，这边拿到默认的WebApplicationContext，即：XmlWebApplicationContext
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		// 1. 校验WebApplicationContext是否已经初始化过，如果已经初始化过，则抛出异常
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		// 3. 实例化contextClass，并强制转成ConfigurableWebApplicationContext，然后返回
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		if (contextClassName != null) {
			try {
				// 2. contextClassName不为空，则使用工具构建出contextClassName的实例
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		// 1. 如果应用上下文id是原始默认值，则根据可用信息生成一个更有用的
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			// The application context id is still set to its original default value
			// -> assign a more useful id based on available information
			// 1.1 从ServletContext中解析初始化参数contextId（可在web.xml中配置）
		...
		// 3. 从servletContext中解析初始化参数contextConfigLocation（可以在web.xml中配置）
		String configLocationParam = sc.getInitParameter(CONFIG_LOCATION_PARAM);
		if (configLocationParam != null) {
			// 4. 设置wac的configLocation属性值为configLocationParam
		...
		if (env instanceof ConfigurableWebEnvironment) {
			// 5. 初始化属性源（主要将servletContextInitParams的占位类替换成sc）
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		// 2. 如果initializerClasses不为空，则遍历处理
	...
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		if (globalClassNames != null) {
			// 1.1 如果globalClassNames不为空，则使用分隔符分割，然后进行遍历
	...
	private Class<ApplicationContextInitializer<ConfigurableApplicationContext>> loadInitializerClass(String className) {
		try {
			...
			// 2. 校验clazz是否实现了ApplicationContextInitializer接口，如果没有则抛出异常
			if (!ApplicationContextInitializer.class.isAssignableFrom(clazz)) {
				throw new ApplicationContextException(
						"Initializer class does not implement ApplicationContextInitializer interface: " + clazz);
			}
			// 3. clazz强制转成ApplicationContextInitializer并返回

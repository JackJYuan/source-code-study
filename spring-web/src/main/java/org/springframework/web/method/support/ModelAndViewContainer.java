 ...
 *
 * 承担着整个请求过程中数据的传递工作。除了保存Model和View外还有一些别的功能
 *
...
public class ModelAndViewContainer {
	...
	/**
	 * 如果为true则在处理器返回redirect视图时一定不会使用defaultModel
	 *
	 * 该值可以在RequestMappingHandlerAdapter中设置。判断处理器返回的是不是redirect视图的标志设置在redirectModelScenario中，
	 * 他是在ReturnValueHandler中设置的，ReturnValueHandler如果判断到是redirect视图就会将redirectModelScenario设置为true。
	 * 也就是说在ReturnValueHandler处理前ModelAndViewContainer的getModel返回的一定是defaultModel，处理后才可能redirectModel
	 * */
	...
	public ModelMap getModel() {
		 ...
		 * 2. 处理器返回的是redirect视图，但是redirectModel为null，而且ignoreDefaultModelOnRedirect为false
		 * 返回redirectModel的情况：
		 * 1. 处理器返回redirect视图，并且redirectModel不为null
		 * 2. 处理器返回的是redirect视图，并且ignoredDefaultModelRedirect为true
		 * */
		if (useDefaultModel()) {

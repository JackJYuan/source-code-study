...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	// Interceptors用于配置SpringMVC的拦截器，有两种设置方式：1，注册HandlerMapping时通过属性设置；2，通过子类的extendInterceptors钩子方法进行设置
	...
	protected void initApplicationContext() throws BeansException {
		// 模版方法，用于给子类提供一个添加/修改Interceptors的入口
		extendInterceptors(this.interceptors);
		// 用于将SpringMVC容器即父容器中所有MappedInterceptor类型的Bean添加到mappedInterceptors属性
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		if (handler == null) {
			// 如果没有获取到则使用默认的Handler，默认的Handler保存在AbstractHandlerMapping的一个Object类型属性defaultHandler，可以在配置HandlerMapping时进行配置
